...
public fun list_hero(nft: Hero, price: u64, ctx: &mut TxContext) {
    ...
    // WARNING: This shares the listing object publicly. Ensure only intended fields are mutable and consider using access control if needed.
...
public fun buy_hero(list_hero: ListHero, coin: Coin<SUI>, ctx: &mut TxContext) {
    ...
    assert!(coin_value == price, EInvalidPayment);
